...
class agepts():
    ...
    def recipes(self, seed: int = 8) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        """
        df = pd.read_csv('\Users\noahz\Downloads\NBA_stats.csv')

        labels = df[['Player', '']]
        #features = df.drop(columns=['player', 'Fields Goals'])
...
if __name__ == '__main__':
    regressor = agepts()
    tr, trl, tst, tstl = regressor.points_age()
    regressor.set_test_data(tst, tstl['Player'])

    regressor.train(pd.concat([tr, tst]), pd.concat(
        [trl[''], tstl['']]))

    for i, title in enumerate(tstl['Player']):
