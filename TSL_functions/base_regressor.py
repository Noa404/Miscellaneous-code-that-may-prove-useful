...
class BaseRegressor():
    ...
    def assess(self, features: pd.DataFrame, labels: pd.Series, titles: pd.Series) -> float:
        ...
        print('Worst performing player at given age',
    ...
    def points_age(self, seed: int = 7) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:

        df = pd.read_csv(r'\Users\noahz\Downloads\NBA_stats.csv')

        labels = df[['Age', 'points']]
